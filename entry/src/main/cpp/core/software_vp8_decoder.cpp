...
#define LOG_TAG "RustDeskSoftVP8"
...
SoftwareVP8Decoder& SoftwareVP8Decoder::instance() {
    static SoftwareVP8Decoder decoder;
    return decoder;
}

SoftwareVP8Decoder::SoftwareVP8Decoder() : worker_(&SoftwareVP8Decoder::workerLoop, this) {}

SoftwareVP8Decoder::~SoftwareVP8Decoder() {
    {
...
void SoftwareVP8Decoder::decodeFrame(const uint8_t* data, int length, bool key, int64_t pts) {
    ...
    if (frames_.size() >= MAX_QUEUED_FRAMES) {
        ...
        if (overflowDrops_ == 1 || overflowDrops_ % 30 == 0) {
            DiagnosticLog::instance().append("W", "vp8_software",
...
void SoftwareVP8Decoder::release() {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool SoftwareVP8Decoder::initializeDecoder() {
    vpx_codec_dec_cfg_t config {};
    const unsigned int hardwareThreads = std::thread::hardware_concurrency();
    config.threads = std::max(2u, std::min(8u, hardwareThreads == 0 ? 4u : hardwareThreads));
    vpx_codec_err_t result = vpx_codec_dec_init(&codec_, vpx_codec_vp8_dx(), &config, 0);
    if (result != VPX_CODEC_OK) {
        DiagnosticLog::instance().append("E", "vp8_software",
...
void SoftwareVP8Decoder::destroyDecoder() {
...
bool SoftwareVP8Decoder::convertFrame(vpx_image_t* image, std::vector<uint8_t>& bgra) {
...
void SoftwareVP8Decoder::workerLoop() {
